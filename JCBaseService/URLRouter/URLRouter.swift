...
//    public var completion: ((Any?) -> Void)?
    ...
    public func registerRoute(_ route: URLRouteName, used subRouter: URLNestingRouterType) {
        ...
        registerRoute(routeUrl.combinedRoute) { routeUrl, _, completion in
            subRouter.openRoute(routeUrl.fullPath, parameters: routeUrl.parameters, completion: completion)
        }
    }
    ...
    public func openRoute(_ route: URLRouteName, parameters: [String: Any]) -> Bool {
        ...
        if let redirectData = routeRedirector.routeRedirectData(from: routeUrl) {
            return openRoute(redirectData.route, parameters: redirectData.params, completion: nil)
        }
        
        // Check if is web link
        if routeUrl.isWebLink {
            if let handler = findRouteHandler(with: routeUrl) {
                return invokeRouteHandler(handler, routeUrl: routeUrl, completion: nil)
            }
            ...
            if let webLinkRouteUrl = routeParser.routeUrl(from: URLRouter.webLink),
               let webLinkHandler = findRouteHandler(with: webLinkRouteUrl) {
                return invokeRouteHandler(webLinkHandler, routeUrl: routeUrl, completion: nil)
            }
            ...
            return upperRouter.openRoute(route, parameters: parameters, completion: nil)
        }
    ...
    public func openRoute(_ route: URLRouteName, parameters: [String: Any], completion: Completion) -> Bool {
        loadDelayedHandlerIfNeed()
        guard let routeUrl = routeParser.routeUrl(from: route, params: parameters) else {
            URLRouterLog("route for (\(route)) is invalid")
            return false
        ...
        // Check if is a redirected route
        if let redirectData = routeRedirector.routeRedirectData(from: routeUrl) {
            return openRoute(redirectData.route, parameters: redirectData.params, completion: completion)
        }
        
        // Check if is web link
        if routeUrl.isWebLink {
            if let handler = findRouteHandler(with: routeUrl) {
                return invokeRouteHandler(handler, routeUrl: routeUrl, completion: completion)
            }
            
            if let webLinkRouteUrl = routeParser.routeUrl(from: URLRouter.webLink),
               let webLinkHandler = findRouteHandler(with: webLinkRouteUrl) {
                return invokeRouteHandler(webLinkHandler, routeUrl: routeUrl, completion: completion)
            }
            ...
            return upperRouter.openRoute(route, parameters: parameters, completion: completion)
        }
    ...
    private func invokeRouteHandler(_ handler: HandleRouteFactory, routeUrl: RouteURL, completion: Completion) -> Bool {
        if routeInterceptor.interceptSuccessfully(for: routeUrl) { return false }
        
        return handler(routeUrl, navigator, completion)
    }
}
